...
class SimpleShader {
    ...
    constructor(vertexShaderPath, fragmentShaderPath) {
        ...
        // Step F: Gets a reference to the uniform variable uOffset in the vertex shader
        this.uOffset = gl.getUniformLocation(this.mCompiledShader, "uOffset");

        // Step G: Gets a reference to the uniform variable uScale in the vertex shader
        this.uScale = gl.getUniformLocation(this.mCompiledShader, "uScale");
    }

    
    // Activate the square shader for rendering
    activateSquare(pixelColor, offset, scale) {
        let gl = core.getGL();
        gl.useProgram(this.mCompiledShader);
        
        // bind vertex buffer
        gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer.getSquare());
        ...
        gl.uniform2fv(this.uScale, scale);
        gl.uniform2fv(this.uOffset, offset);
    }

    // Activate the triangle shader for rendering
    activateTriangle(pixelColor, offset, scale) {
        let gl = core.getGL();
        gl.useProgram(this.mCompiledShader);
        
        // bind vertex buffer
        gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer.getTriangle());
        gl.vertexAttribPointer(this.mVertexPositionRef,
            3,              // each element is a 3-float (x,y.z)
            gl.FLOAT,       // data type is FLOAT
            false,          // if the content is normalized vectors
    ...
    // Activate the circle shader for rendering
    activateCircle(pixelColor, offset, scale) {
        let gl = core.getGL();
        gl.useProgram(this.mCompiledShader);
        
        // bind vertex buffer
        gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer.getCircle());
        gl.vertexAttribPointer(this.mVertexPositionRef,
            3,              // each element is a 3-float (x,y.z)
            gl.FLOAT,       // data type is FLOAT
            false,          // if the content is normalized vectors
